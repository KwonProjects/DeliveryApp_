package co.kr.kwon.deliveryapp.screen.review.camera.preview
...
import co.kr.kwon.deliveryapp.databinding.ActivityImagePreviewListBinding
import co.kr.kwon.deliveryapp.util.path.PathUtil
import co.kr.kwon.deliveryapp.widget.adapter.ImageViewPagerAdapter
import java.io.File
...
class ImagePreviewListActivity : AppCompatActivity() {
    ...
    private fun removeImage(uri: Uri) {
        ...
        if (imageViewPagerAdapter.uriList.isEmpty()) {
            Toast.makeText(this@ImagePreviewListActivity, "삭제할 수 있는 이미지가 없습니다.", Toast.LENGTH_SHORT).show()
            finish()
        }
    }
